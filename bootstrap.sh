...
sudo docker run --name bf-aws-dev -it local:aws-dev /bin/bash -c "source /usr/local/bin/session-tool.sh; get_session -i /root/secrets.csv -b bf-aws-tools-session-tool -d"
